...
const User = () => {
    ...
    const { name, type, avatar_url, location, bio, blog,
        email, followers, twitter_username,
        login, html_url, following, follwers,
        public_repos, public_gists, hireable } = state.user
    ...
    return (
        <motion.div className='w-full flex justify-center lg:w-10/12 p-5 md:p-6  items-center flex-col'
            initial={{ y: 50 }}
            animate={{ y: 0 }}  >
            <div className='mb-4 w-full'>
                <Link to="/" className='btn btn-ghost hover:bg-transparent hover:text-primary'>Back To Home</Link>
            </div>
            ...
            <div className='flex justify-center  w-full  py-6 transition-all duration-200 flex-col  '>
                <div className={`${theme ? " shadow-md   w-full flex flex-col max-width-custom bg-[#fcfcfc] p-5 gap-5 rounded-md" : "shadow-lg flex-col  w-full flex max-width-custom bg-base-300 p-5 gap-5 rounded-md"} `} >
                    <div  className='flex w-full gap-3  max-width-user '>
                        ...
                        <div className="avatar w-60   transition-all duration-200 max-width-img-container ">
                            ...
                            <div className={`rounded-full ring  self-baseline    ring-offset-2 max-width-img max-md ${theme ? "ring-indigo-400 ring-offset-gray-100 ": "ring-indigo-900 ring-offset-gray-800"}`}   >
                                <img className='transition-all duration-200' src={avatar_url} />
                                <div className='absolute bottom-3 right-5 w-4/5 max-w-[80%] block opacity-0 transition-opacity duration-300 '>
                        ...
                        <div className='w-full   px-5  py-3 max-width-padding  '>
                            <h1 className='flex gap-2 items-baseline flex-wrap max-width-name-container mb-4'>
                                ...
                                <div className=' flex flex-col text-2xl font-bold mr-3'>{name}
                                    <p className="text-sm text-ellipsis whitespace-nowrap z-10  opacity-60 ">{login}</p>
                                </div>

                                <div className='flex  gap-2 mt-1'>
                                    {hireable && <div className=" badge badge-sm badge-outline badge-success pt-1 pb-[0.4rem]">hireable</div>}

                                    <div className="  badge badge-sm badge-primary badge-outline  pt-1 pb-[0.4rem]">{type}</div>
                                    {+followers >= 1000 && <div className="  badge badge-sm badge-accent badge-outline  pt-1 pb-[0.4rem]">popular</div>}
                            ...
                            <div  className=''>
                                ...
                                <div className="mt-7 card-actions">
                                    <a className='btn btn-md btn-outline w-full  sm:btn sm:btn-outline sm:w-fit !border-2  ' href={html_url}
                                        target='_blank'
                                        rel="noreferrer"
                            ...
                            {!email && !blog && !location === true ? "" :
                                <div ref={ref} className='w-full border-b-2 border-b-base-content  shadow-base-200 mt-10 flex justify-between align-middle gap-5 flex-wrap '>
                                    ...
                                    {!twitter_username && !email === true ? "" :
                                        <>
                                            ...
                                            </div> : <div className=' flex-1 pb-3 px-4  '>
